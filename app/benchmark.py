...
def measure_accuracy(model: torch.nn.Module, loader: DataLoader, device: torch.device, optimization: OptimizationName) -> Tuple[float, float]:
	top1_correct = 0
	top5_correct = 0
	total = 0
	with torch.no_grad():
...
def main():
	...
	parser.add_argument(
		"--modes",
		type=str,
		default="baseline,amp,channels_last,compile,jit",
